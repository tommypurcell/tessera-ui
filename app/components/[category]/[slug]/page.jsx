...
const categoryBackLabels = { custom: 'Custom Made Components', 'building-blocks': 'Building Blocks' }

export function generateStaticParams() {
  return getCollectionNames().flatMap((category) =>
    getCollection(category).map((component) => ({ category, slug: component.slug })),
  )
}
  ...
  return (
    <>
      ...
      <BaseHero
        ...
        backLabel={`Back to ${categoryBackLabels[category] ?? `${category} components`}`}
      />
      <main id="main-content" className="mx-auto max-w-7xl px-4 py-10">
        <div className="mb-8 flex items-center gap-2 text-sm text-gray-600">
          <span>
            {previews.filter((item) => item.dark).length}/{component.components.length} dark-mode
            variants
          </span>
            ...
            <>
              ...
              <span>
                ...
                {new Intl.DateTimeFormat('en-US', { dateStyle: 'medium', timeZone: 'UTC' }).format(
                  new Date(component.updated.date),
                )}
              </span>
            </>
          )}
        ...
        <div className="space-y-8">
          {previews.map((preview) => (
            <ComponentPreview
              ...
              themeVisibility={
                preview.dark ? 'hidden dark:block' : preview.hasDarkVariant ? 'dark:hidden' : undefined
              }
            />
          ))}
        </div>
