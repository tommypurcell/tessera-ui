...
export type ApplicantCardVariant1DarkProps = {
  initials: string
  name: string
  roleApplied: string
  rating: number
 ...
 */
export function ApplicantCardDark({ initials, name, roleApplied, rating, resumeLabel = 'Resume.pdf', resumeHref = '#', stageLabel }: ApplicantCardVariant1DarkProps) {
  return (
    <div className="w-full max-w-sm rounded-lg border border-gray-700 bg-gray-900 p-4">
      <div className="flex items-start gap-3">
        <span className="flex size-11 shrink-0 items-center justify-center rounded-full bg-gray-100 text-sm font-medium text-gray-900">{initials}</span>
        <div className="min-w-0 flex-1">
          <p className="text-sm font-semibold text-white">{name}</p>
          <p className="text-xs text-gray-400">Applied for {roleApplied}</p>

          <div className="mt-1.5 flex items-center gap-0.5" role="img" aria-label={`Rated ${rating} out of 5 stars`}>
            {Array.from({ length: 5 }, (_, index) => (
              <svg key={index} aria-hidden="true" className={`size-3.5 ${index < rating ? 'text-amber-400' : 'text-gray-600'}`} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
      ...
      <div className="mt-3 flex items-center justify-between border-t border-gray-800 pt-3">
        <a href={resumeHref} className="inline-flex items-center gap-1.5 text-xs font-medium text-gray-300 hover:text-white hover:underline">
        ...
        <span className="rounded-full bg-blue-400/10 px-2 py-0.5 text-xs font-medium text-blue-300">{stageLabel}</span>
      </div>
    </div>
  )
}
