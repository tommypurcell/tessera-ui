...
export type AppointmentSlotCardVariant1Props = Omit<HTMLAttributes<HTMLDivElement>, 'children'> & {
  providerName: string
...
export function AppointmentSlotCard({
  ...
  onConfirm,
  ...props
}: AppointmentSlotCardVariant1Props) {
  return (
    <div className={`rounded-xl border border-gray-200 bg-white p-5 ${className ?? ''}`} {...props}>
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-3">
          <span className="flex size-10 shrink-0 items-center justify-center rounded-full bg-gray-200 text-sm font-medium text-gray-600">{providerInitials}</span>
          <div>
            <p className="text-sm font-semibold text-gray-900">{providerName}</p>
            <p className="text-xs text-gray-500">{providerSpecialty}</p>
          </div>
        </div>
        <span className="inline-flex items-center rounded-full bg-blue-100 px-2 py-0.5 text-xs font-medium text-blue-700">{appointmentType}</span>
      </div>

      <div className="mt-4 flex items-center gap-2 rounded-lg bg-gray-50 px-3 py-2.5 text-sm text-gray-700">
      ...
      <div className="mt-4 flex gap-2.5">
        <button type="button" onClick={onReschedule} className="flex-1 rounded-md border border-gray-300 bg-white px-3.5 py-2 text-sm font-medium text-gray-700 shadow-sm hover:bg-gray-50">
          {rescheduleLabel}
        </button>
