...
export type BulletChartVariant1DarkProps = {
  label: string
  value: number
  target: number
  max: number
...
const bandShades = ['bg-gray-700', 'bg-gray-800', 'bg-gray-800/60']

/**
 * Copy-and-own Tailwind component. Single-metric bullet graph adapted for
 * dark surfaces — qualitative background bands, a value bar, and a target
 * marker, with every position computed as a percentage of `max`.
 */
export function BulletChartDark({ label, value, target, max, bands }: BulletChartVariant1DarkProps) {
    ...
    <div className="flex flex-col gap-2 rounded-lg border border-gray-800 bg-gray-900 p-6">
      <div className="flex items-baseline justify-between text-sm">
        ...
        <span className="font-semibold text-gray-100">
          {value.toLocaleString()} <span className="font-normal text-gray-500">/ {target.toLocaleString()} target</span>
        </span>
      </div>
      ...
      <div className="relative h-6 w-full overflow-hidden rounded-sm">
        ...
        >
          <div className="ml-auto h-3.5 w-1.5 -mr-0.5 rounded-sm bg-blue-300" />
        </div>

        <div
          className="absolute inset-y-1 w-0.5 bg-gray-100"
