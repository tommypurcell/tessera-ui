...
export type CallLogRowVariant1Props = {
  calls: CallLogEntry[]
  className?: string
}
...
const directionMeta: Record<CallDirection, { label: string; icon: () => JSX.Element; iconClassName: string; textClassName: string }> = {
  incoming: { label: 'Incoming', icon: IncomingIcon, iconClassName: 'text-emerald-500', textClassName: 'text-gray-500' },
  outgoing: { label: 'Outgoing', icon: OutgoingIcon, iconClassName: 'text-blue-500', textClassName: 'text-gray-500' },
  missed: { label: 'Missed', icon: IncomingIcon, iconClassName: 'text-red-500', textClassName: 'text-red-500' },
}

/**
 * Copy-and-own Tailwind component. Call log row — contact avatar, a direction icon
 * (incoming/outgoing/missed) whose color and glyph are both derived from the same
 * direction value, duration, timestamp, and a real callback action.
 */
export function CallLogRowVariant1({ calls, className }: CallLogRowVariant1Props) {
  return (
        ...
        {calls.map((call) => {
          ...
          return (
            <li key={call.id} className="flex items-center gap-3 px-4 py-3">
              <div aria-hidden="true" className="flex size-9 shrink-0 items-center justify-center rounded-full bg-blue-100 text-sm font-semibold text-blue-700">
                {call.initials}
              </div>
              ...
              <button
                ...
                className="flex size-8 shrink-0 items-center justify-center rounded-full text-gray-400 hover:bg-gray-100 hover:text-gray-600"
              >
