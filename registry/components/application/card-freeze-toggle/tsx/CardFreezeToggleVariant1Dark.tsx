...
export type CardFreezeToggleVariant1DarkProps = {
...
/**
 * Copy-and-own Tailwind component. Dark-surface variant of the card freeze toggle.
 */
export function CardFreezeToggleVariant1Dark({
  cardholderName,
  lastFour,
  expiry,
  initialFrozen = false,
  onFrozenChange,
  className,
}: CardFreezeToggleVariant1DarkProps) {
    ...
    <div className={`rounded-xl border border-gray-800 bg-gray-950 p-5 shadow-sm ${className ?? ''}`}>
      ...
      <div className="mt-4 flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-gray-400">{frozen ? 'Card is frozen' : 'Card is active'}</p>
        </div>
        ...
        <button
          ...
          className={`relative h-6 w-11 shrink-0 rounded-full transition-colors ${frozen ? 'bg-red-500' : 'bg-gray-700'}`}
        >
