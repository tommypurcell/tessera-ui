...
export type ComboboxVariant1DarkProps = {
  label: string
 ...
 * adapted for dark surfaces, with a listbox of matches and a selected/no-match state.
 */
export function ComboboxVariant1Dark({
  label,
  options = DEFAULT_OPTIONS,
  placeholder = 'Search…',
  onSelect,
  className,
}: ComboboxVariant1DarkProps) {
    ...
    <div className={`flex flex-col gap-1.5 ${className ?? ''}`}>
      ...
      <div className="relative">
        <input
          ...
          className="w-full rounded-md border border-gray-700 bg-gray-900 py-2 pl-3 pr-9 text-sm text-white shadow-sm focus:border-gray-500 focus:outline-none"
        />
        ...
        {open ? (
          <ul
            ...
            role="listbox"
            aria-label={label}
            className="absolute z-10 mt-1.5 w-full overflow-hidden rounded-md border border-gray-800 bg-gray-900 py-1 shadow-lg"
          >
            {filtered.length > 0 ? (
              filtered.map((option) => {
                ...
                return (
                  <li
                    ...
                    className={
                      isSelected
                        ? 'flex cursor-pointer items-center justify-between gap-2 bg-blue-950 px-3 py-1.5 text-sm text-blue-300'
                        : 'cursor-pointer px-3 py-1.5 text-sm text-gray-200 hover:bg-gray-800'
                    }
                  >
