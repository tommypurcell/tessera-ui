...
export type ComfortableListRowVariant1DarkProps = {
 ...
 */
export function ComfortableListRowDark({ items }: ComfortableListRowVariant1DarkProps) {
  return (
    <ul className="w-full max-w-md divide-y divide-gray-800 rounded-xl border border-gray-700 bg-gray-900 shadow-sm">
      {items.map((item) => (
        <li key={item.id}>
          <a href={item.href ?? '#'} className="flex items-center gap-4 p-5 hover:bg-gray-800">
            <img src={item.avatarUrl} alt="" className="size-12 shrink-0 rounded-full" />
            <div className="min-w-0 flex-1">
              <p className="text-sm font-semibold text-white">{item.title}</p>
              <p className="mt-0.5 truncate text-sm text-gray-400">{item.subtitle}</p>
            </div>
