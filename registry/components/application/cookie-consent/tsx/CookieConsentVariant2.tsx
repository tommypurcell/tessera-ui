...
export type CookieConsentVariant2Props = {
 ...
 * Pairs with CookieConsent's "Customize" button.
 */
export function CookieConsentPreferences({ categories, onCategoryChange, onBack, onSave }: CookieConsentVariant2Props) {
  return (
    <div role="dialog" aria-label="Cookie preferences" className="w-full border-t border-gray-200 bg-white p-5 shadow-[0_-4px_16px_-4px_rgba(0,0,0,0.08)]">
      <div className="mx-auto flex max-w-2xl flex-col gap-4">
        <div>
          <h2 className="text-sm font-semibold text-gray-900">Manage cookie preferences</h2>
          <p className="mt-1 text-sm text-gray-500">Essential cookies are always on. Choose which other categories to allow.</p>
        </div>
          ...
          {categories.map((category) => (
            <div key={category.id} className="flex items-center justify-between gap-4 p-3.5">
              ...
              >
                ...
                />
                <span className={`absolute inset-0 m-auto h-2 rounded-full ${category.checked ? 'bg-gray-900' : 'bg-gray-300'}`} />
                <span
