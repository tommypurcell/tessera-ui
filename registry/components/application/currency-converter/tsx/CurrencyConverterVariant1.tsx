...
export type CurrencyConverterVariant1Props = {
  fromAmount: string
  fromCurrency: string
  toCurrency: string
  currencies: string[]
...
export function CurrencyConverter({
  ...
  onToCurrencyChange,
  onSwap,
}: CurrencyConverterVariant1Props) {
  const fromId = useId()
  const toId = useId()
  ...
  return (
    <div className="w-full max-w-xs">
      <label className="mb-1.5 block text-sm font-medium text-gray-700" htmlFor={fromId}>
        You send
      </label>
      ...
      <label className="mb-1.5 block text-sm font-medium text-gray-700" htmlFor={toId}>
        You receive
      </label>
      <div className="flex items-center rounded-md border border-gray-300 bg-gray-50 shadow-sm">
        <input type="text" inputMode="decimal" id={toId} value={converted} readOnly className="h-10 w-full border-0 bg-transparent px-3 text-sm text-gray-900 focus:outline-none focus:ring-0" />
