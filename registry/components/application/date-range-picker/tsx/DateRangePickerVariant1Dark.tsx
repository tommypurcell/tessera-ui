...
export type DateRangePickerVariant1DarkProps = Omit<HTMLAttributes<HTMLDivElement>, 'children'> & {
  label?: string
...
export function DateRangePickerDark({
  ...
  onApply,
  ...props
}: DateRangePickerVariant1DarkProps) {
  return (
    <div className={`w-full max-w-md ${className ?? ''}`} {...props}>
      <label htmlFor="date-range-input" className="mb-1.5 block text-sm font-medium text-gray-300">
        {label}
      </label>
      ...
      {open ? (
        <div role="dialog" aria-label="Choose date range" className="relative mt-2 flex w-full overflow-hidden rounded-xl border border-gray-800 bg-gray-900 shadow-lg shadow-black/40">
          <div className="flex w-36 shrink-0 flex-col gap-0.5 border-r border-gray-800 p-2">
            {presets.map((preset) => (
              <button
                key={preset.id}
                type="button"
                ...
                className={`rounded-md px-2.5 py-1.5 text-left text-sm ${
                  preset.active ? 'bg-gray-800 font-medium text-white' : 'text-gray-400 hover:bg-gray-800'
                }`}
              >
                {preset.label}
              </button>
          ...
          <div className="flex-1 p-3">
                ...
                <button
                  ...
                  className={`mx-auto flex size-8 items-center justify-center text-sm ${
                    day.isRangeStart || day.isRangeEnd
                      ? 'rounded-full bg-white font-medium text-gray-900'
                      : day.isInRange
                        ? 'w-full bg-gray-800 text-gray-200'
                        : day.isOutsideMonth
                          ? 'rounded-full text-gray-600 hover:bg-gray-800'
                          : 'rounded-full text-gray-300 hover:bg-gray-800'
                  }`}
                >
                  {day.date}
                </button>
            ...
            <div className="mt-3 flex items-center justify-between border-t border-gray-800 pt-3">
              <span className="text-xs text-gray-400">{nightsLabel}</span>
              <div className="flex gap-2">
                <button type="button" onClick={onCancel} className="rounded-md border border-gray-700 px-3 py-1.5 text-xs font-medium text-gray-300 hover:bg-gray-800">
                  Cancel
                </button>
