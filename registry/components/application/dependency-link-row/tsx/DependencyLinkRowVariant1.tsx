...
export type DependencyLinkRowVariant1Props = {
  blocks: LinkedIssue[]
 ...
 */
export function DependencyLinkRow({ blocks, blockedBy }: DependencyLinkRowVariant1Props) {
  return (
    <div className="flex flex-col gap-4 rounded-lg border border-gray-200 bg-white p-4">
      {blocks.length > 0 ? (
        <div>
          <p className="mb-1.5 flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wide text-gray-500">
            <svg aria-hidden="true" className="size-3.5 text-red-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
          ...
          <div className="flex flex-wrap gap-1.5">
            {blocks.map((issue) => (
              <a key={issue.id} href={issue.href} className="inline-flex items-center gap-1.5 rounded-md border border-gray-200 bg-white px-2 py-1 text-xs font-medium text-gray-700 hover:bg-gray-50">
      ...
      {blockedBy.length > 0 ? (
        <div>
          <p className="mb-1.5 flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wide text-gray-500">
            <svg aria-hidden="true" className="size-3.5 text-amber-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
              <path
                strokeLinecap="round"
