export type DishCardVariant2Props = {
  imageUrl: string
  imageAlt: string
  name: string
...
/**
 * Copy-and-own Tailwind component. Horizontal menu-row dish card with a
 * spice-level indicator and a quantity stepper for an item already added to
 * the order — the food-delivery list-row counterpart to Variant 1's card.
 */
export function DishCard({
  ...
  onDecrease,
  onIncrease,
}: DishCardVariant2Props) {
  return (
    <div className="flex w-full max-w-md items-center gap-3 rounded-lg border border-gray-200 bg-white p-3">
      ...
      <div className="flex shrink-0 items-center gap-2 rounded-md border border-gray-200 px-1 py-1">
        <button
          type="button"
          aria-label="Decrease quantity"
          onClick={onDecrease}
          className="flex size-6 items-center justify-center rounded text-gray-500 hover:bg-gray-100"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            className="size-3.5"
