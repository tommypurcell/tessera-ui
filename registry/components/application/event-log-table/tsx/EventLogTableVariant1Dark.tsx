...
export type EventLogTableVariant1DarkProps = Omit<HTMLAttributes<HTMLDivElement>, 'children'> & {
  rows: EventLogRow[]
  onToggle?: (row: EventLogRow) => void
}

const typeStylesDark: Record<EventLogType, { badge: string; icon: ReactNode }> = {
  success: {
 ...
 */
export function EventLogTableDark({ className, rows, onToggle, ...props }: EventLogTableVariant1DarkProps) {
  return (
              ...
              <tr key={row.id}>
                ...
                <td className="whitespace-nowrap px-2 py-3">
                  <div className="flex items-center gap-2">
                    <span className={`flex size-5 shrink-0 items-center justify-center rounded ${typeStylesDark[row.type].badge}`}>
                      <svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="2" stroke="currentColor" className="size-3">
                        {typeStylesDark[row.type].icon}
                      </svg>
                    </span>
                    <code className="text-sm font-medium text-white">{row.name}</code>
                  </div>
                </td>
                <td className="whitespace-nowrap px-2 py-3 text-sm text-gray-400">{row.timestamp}</td>
              </tr>
              {row.expanded ? (
                <tr key={`${row.id}-payload`}>
                  <td colSpan={3} className="bg-gray-800 px-3 py-3">
                    <pre className="overflow-x-auto pl-8 font-mono text-[13px] leading-relaxed text-gray-400">{row.payload}</pre>
                  </td>
                </tr>
              ) : null}
            </>
