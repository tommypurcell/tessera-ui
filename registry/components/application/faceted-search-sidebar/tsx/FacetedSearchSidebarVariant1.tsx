...
export type FacetedSearchSidebarVariant1Props = {
...
/**
 * Copy-and-own Tailwind component. Collapsible facet groups — checkbox options with
 * result counts, and a two-thumb price range — for e-commerce/search filtering.
 */
export function FacetedSearchSidebarVariant1({
  ...
  onClearAll,
  className,
}: FacetedSearchSidebarVariant1Props) {
  ...
  return (
    <nav
      aria-label="Filters"
      className={`flex flex-col divide-y divide-gray-100 rounded-xl border border-gray-200 bg-white shadow-sm ${className ?? ''}`}
    >
      <FacetGroup title="Category" defaultOpen>
        <div className="flex flex-col gap-2.5">
          {categoryOptions.map((option) => (
            <label key={option.id} className="flex items-center justify-between gap-2 text-sm text-gray-700">
      ...
      <FacetGroup title="Price" defaultOpen>
        <div className="flex flex-col gap-3">
          <div className="relative h-1.5 rounded-full bg-gray-200">
            ...
            />
            <span
              className="absolute -top-1 size-3.5 -translate-x-1/2 rounded-full border-2 border-blue-600 bg-white"
              style={{ left: `${leftPct}%` }}
            />
            <span
              className="absolute -top-1 size-3.5 translate-x-1/2 rounded-full border-2 border-blue-600 bg-white"
      ...
      <FacetGroup title="Brand" defaultOpen={false}>
        <p className="text-xs text-gray-400">No brand filters available yet.</p>
      </FacetGroup>
