...
export type FileTreeVariant1DarkProps = {
  nodes: FileTreeNode[]
  className?: string
}
...
function Chevron({ open }: { open: boolean }) {
  return (
    <svg className={`h-3.5 w-3.5 shrink-0 text-gray-500 transition-transform ${open ? '' : '-rotate-90'}`} viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
      <path
        fillRule="evenodd"
...
function TreeNode({ node }: { node: FileTreeNode }) {
      ...
      <li role="treeitem">
        <a
          ...
          className={`flex items-center gap-1.5 rounded px-1.5 py-1 ${
            node.current ? 'bg-indigo-500/10 text-indigo-300' : 'text-gray-300 hover:bg-gray-800'
          }`}
        >
  ...
  return (
    <li role="treeitem" aria-expanded={open}>
      <button type="button" onClick={() => setOpen((v) => !v)} className="flex w-full items-center gap-1.5 rounded px-1.5 py-1 text-left hover:bg-gray-800">
        <Chevron open={open} />
        <FolderIcon />
        <span className="font-medium text-gray-100">{node.name}</span>
      </button>
      {open ? (
        <ul role="group" className="ml-3 flex flex-col border-l border-gray-800 pl-2">
...
/**
 * Copy-and-own Tailwind component (dark surface). Collapsible nested directory/file
 * explorer with indent guides. Folders expand/collapse independently; files render
 * as real links and support an aria-current="page" state for the active file.
 */
export function FileTree({ nodes, className }: FileTreeVariant1DarkProps) {
  return (
    <div className={`rounded-lg border border-gray-800 bg-gray-900 p-2 ${className ?? ''}`}>
