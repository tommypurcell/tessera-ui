...
export type FloatingActionMenuVariant2DarkProps = Omit<HTMLAttributes<HTMLDivElement>, 'children'> & {
 ...
 */
export function FloatingActionMenuVariant2Dark({
  className,
  children,
  renderContent,
  before,
  ...
  errorContent,
  ...props
}: FloatingActionMenuVariant2DarkProps) {
  const defaultContent = (
    <>
      <div className="group relative">
            <input type="checkbox" id="fab2-toggle-dark" className="peer sr-only" checked />
            ...
            <div className="pointer-events-none absolute right-1.5 bottom-full mb-4 flex flex-col items-center gap-3 opacity-0 transition-all duration-150 peer-checked:pointer-events-auto peer-checked:opacity-100">
              <button
                type="button"
                aria-label="Share"
                className="flex size-10 items-center justify-center rounded-full bg-gray-800 text-gray-200 shadow-md ring-1 ring-white/10 transition hover:bg-gray-700"
              >
            ...
            <label
              htmlFor="fab2-toggle-dark"
              className="flex size-14 cursor-pointer items-center justify-center rounded-full bg-gray-100 text-gray-900 shadow-lg transition hover:bg-white peer-checked:rotate-45"
            >
