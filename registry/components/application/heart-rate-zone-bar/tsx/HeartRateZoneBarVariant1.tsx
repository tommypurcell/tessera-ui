...
/**
 * Copy-and-own Tailwind component. Segment widths are each zone's share of
 * total minutes; the current-zone marker is positioned at the midpoint of
 * that zone's own segment, all computed from real minute values.
 */
export function HeartRateZoneBar({ zones, currentZone, currentBpm }: HeartRateZoneBarVariant1Props) {
  ...
  return (
    <div className="flex w-full max-w-md flex-col gap-3 rounded-lg border border-gray-200 bg-white p-5">
      <div className="flex items-baseline justify-between">
        <h2 className="text-sm font-medium text-gray-900">Heart rate zones</h2>
      ...
      <div className="relative pt-4">
        ...
        >
          <span className="whitespace-nowrap rounded bg-gray-900 px-1.5 py-0.5 text-[10px] font-semibold text-white">
            {currentBpm} bpm
          </span>
          <span aria-hidden="true" className="mt-0.5 h-2 w-px bg-gray-900" />
        </div>
      ...
      <div className="flex flex-wrap items-center gap-x-4 gap-y-1.5 text-xs text-gray-500">
        {segments.map((s) => (
          <span key={s.zone} className="flex items-center gap-1.5">
            ...
            <span className={s.zone === currentZone ? 'font-semibold text-gray-900' : ''}>
