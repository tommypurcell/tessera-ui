...
export type LiveMatchTimelineVariant1DarkProps = {
  homeTeam: string
  awayTeam: string
...
const eventIcon: Record<MatchEventType, { label: string; className: string }> = {
  goal: { label: 'Goal', className: 'text-emerald-400' },
  'yellow-card': { label: 'Yellow card', className: '' },
  substitution: { label: 'Substitution', className: 'text-blue-400' },
}
...
/**
 * Copy-and-own Tailwind component (dark surface). Live sports match event
 * timeline: a horizontal minute axis with goal/card/substitution markers
 * plotted at their minute and alternating above/below the line by team
 * side.
 */
export function LiveMatchTimeline({ homeTeam, awayTeam, currentMinute, matchLengthMinutes = 90, events, className }: LiveMatchTimelineVariant1DarkProps) {
  return (
    <div className={`rounded-xl border border-gray-800 bg-gray-900 p-5 ${className ?? ''}`}>
      <div className="flex items-center justify-between text-sm">
        <span className="font-semibold text-gray-100">{homeTeam}</span>
        <span className="rounded-full bg-emerald-500/10 px-2 py-0.5 text-xs font-medium text-emerald-400">
          Live &middot; {currentMinute}&prime;
        </span>
        <span className="font-semibold text-gray-100">{awayTeam}</span>
      </div>

      <div className="relative mt-8 h-24">
        <div className="absolute left-0 right-0 top-1/2 h-0.5 -translate-y-1/2 bg-gray-800"></div>
        ...
        {events.map((event) => {
          ...
          return (
            ...
            >
              {isHome ? (
                <>
                  <span className="text-[11px] font-medium text-gray-400">{event.minute}&prime;</span>
                  <EventIcon type={event.type} />
                  <div className="h-2.5 w-0.5 bg-gray-700"></div>
                </>
              ) : (
                <>
                  <div className="h-2.5 w-0.5 bg-gray-700"></div>
