...
export type LiveStreamBadgeVariant1DarkProps = Omit<HTMLAttributes<HTMLDivElement>, 'children'> & {
 ...
 */
export function LiveStreamBadgeVariant1Dark({
  className,
  children,
  renderContent,
  before,
  ...
  errorContent,
  ...props
}: LiveStreamBadgeVariant1DarkProps) {
  const defaultContent = (
    <>
      <div className="relative w-full max-w-xs overflow-hidden rounded-lg shadow-sm">
            ...
            <p className="mt-1.5 truncate px-0.5 text-sm font-medium text-white">Building a design system from scratch</p>
          </div>
    </>
  )
  const content =
