...
export type MetricSparklineCardVariant1Props = Omit<HTMLAttributes<HTMLElement>, 'children'> & {
  label: string
  value: string | number
...
const directionStyles: Record<MetricSparklineCardDelta['direction'], { badge: string; stroke: string; fill: string }> = {
  up: { badge: 'bg-green-100 text-green-600', stroke: 'stroke-green-500', fill: 'fill-green-500/10' },
  down: { badge: 'bg-red-100 text-red-600', stroke: 'stroke-red-500', fill: 'fill-red-500/10' },
}
...
/**
 * Copy-and-own Tailwind component. KPI tile pairing a big number and delta
 * badge with an inline area-fill sparkline computed from a typed points array
...
export function MetricSparklineCard({
  ...
  rangeLabel,
  ...props
}: MetricSparklineCardVariant1Props) {
  ...
  return (
    <article
      className={`w-72 rounded-lg border border-gray-100 bg-white p-6 ${className ?? ''}`}
      {...props}
    >
      ...
      <p className="mt-2 text-xs text-gray-400">{rangeLabel}</p>
    </article>
  )
}
