...
export type MetricSparklineCardVariant2Props = Omit<HTMLAttributes<HTMLElement>, 'children'> & {
  label: string
  value: string | number
...
const directionStyles: Record<MetricSparklineCardDelta['direction'], { badge: string; stroke: string; dot: string }> = {
  up: { badge: 'bg-green-100 text-green-600', stroke: 'stroke-green-500', dot: 'fill-green-500' },
  down: { badge: 'bg-red-100 text-red-600', stroke: 'stroke-red-500', dot: 'fill-red-500' },
}
...
/**
 * Copy-and-own Tailwind component. KPI tile with a line-only sparkline and a
 * marker dot on the latest value, computed from a typed points array.
 */
export function MetricSparklineCard({
  ...
  rangeLabel,
  ...props
}: MetricSparklineCardVariant2Props) {
