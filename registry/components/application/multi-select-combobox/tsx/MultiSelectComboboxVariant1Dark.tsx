...
export type MultiSelectComboboxVariant1DarkProps = {
  label: string
 ...
 */
export function MultiSelectComboboxDark({
  label,
  options = DEFAULT_OPTIONS,
  placeholder = 'Search…',
  onChange,
  className,
}: MultiSelectComboboxVariant1DarkProps) {
    ...
    <div className={`flex flex-col gap-1.5 ${className ?? ''}`}>
      ...
      <div className="relative">
        <div
          ...
          className="flex flex-wrap items-center gap-1.5 rounded-md border border-gray-700 bg-gray-900 p-1.5 focus-within:border-gray-500"
        >
          {selected.map((option) => (
            <span key={option.value} className="inline-flex items-center gap-1 rounded-full bg-blue-500/10 py-0.5 pr-1 pl-2.5 text-sm text-blue-300">
              {option.label}
              <button
                type="button"
        ...
        {open ? (
          <ul
            ...
            className="absolute z-10 mt-1.5 w-full overflow-hidden rounded-md border border-gray-700 bg-gray-900 py-1 shadow-lg"
          >
            {filtered.length > 0 ? (
              filtered.map((option) => {
                const isSelected = selected.some((o) => o.value === option.value)
                return (
                  <li
                    ...
                    className={
                      isSelected
                        ? 'flex cursor-pointer items-center justify-between gap-2 bg-blue-500/10 px-3 py-1.5 text-sm text-blue-300'
                        : 'flex cursor-pointer items-center justify-between gap-2 px-3 py-1.5 text-sm text-gray-300 hover:bg-gray-800'
                    }
                  >
