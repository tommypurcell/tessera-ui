...
/**
 * Copy-and-own Tailwind component. Full-width bottom-docked player bar —
 * distinct from audio-player, which is a card-style waveform player.
 * Scrubber fill is elapsedSeconds/durationSeconds, computed live.
 */
export function NowPlayingBar({ title, artist, artUrl, elapsedSeconds, durationSeconds, isPlaying = true }: NowPlayingBarVariant1Props) {
  ...
  return (
    <div className="flex w-full items-center gap-4 border-t border-gray-200 bg-white px-4 py-3">
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-baseline justify-between gap-2">
          ...
          <div className="flex shrink-0 items-center gap-1 text-[11px] tabular-nums text-gray-400">
        ...
        <div
          className="mt-2 h-1 w-full rounded-full bg-gray-200"
          role="img"
          aria-label={`Playback progress: ${formatTime(elapsedSeconds)} of ${formatTime(durationSeconds)}`}
        >
          <div className="h-1 rounded-full bg-gray-900" style={{ width: `${progressPct}%` }} />
        </div>
      </div>

      <div className="flex shrink-0 items-center gap-1">
        <button type="button" aria-label="Previous track" className="inline-flex size-8 items-center justify-center rounded-full text-gray-500 hover:bg-gray-100 hover:text-gray-900">
        ...
        <button
          ...
          className="inline-flex size-10 items-center justify-center rounded-full bg-gray-900 text-white hover:bg-gray-700"
        >
          {isPlaying ? (
        ...
        <button type="button" aria-label="Next track" className="inline-flex size-8 items-center justify-center rounded-full text-gray-500 hover:bg-gray-100 hover:text-gray-900">
          <svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="size-4">
