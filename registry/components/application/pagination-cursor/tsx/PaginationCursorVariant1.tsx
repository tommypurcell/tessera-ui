...
export type PaginationCursorVariant1Props = Omit<HTMLAttributes<HTMLDivElement>, 'children'> & {
  rangeStart: number
  rangeEnd: number
  total: number
  pageSize: number
...
export function PaginationCursor({
  ...
  onNext,
  ...props
}: PaginationCursorVariant1Props) {
  return (
    <div className={`flex items-center justify-between gap-4 rounded-xl border border-gray-200 bg-white px-4 py-3 ${className ?? ''}`} {...props}>
      ...
      <div className="flex items-center gap-3">
        <label className="flex items-center gap-1.5 text-sm text-gray-500">
          ...
          <select
            ...
            className="rounded-md border border-gray-300 px-2 py-1 text-sm text-gray-900 shadow-sm focus:border-gray-500 focus:outline-none"
          >
