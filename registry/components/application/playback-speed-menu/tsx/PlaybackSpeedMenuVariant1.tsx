...
function CheckIcon({ visible }: { visible: boolean }) {
  return (
    <svg
      ...
      strokeWidth="2.5"
      stroke="currentColor"
      className={`size-3.5 text-blue-600 ${visible ? '' : 'invisible'}`}
    >
...
export type PlaybackSpeedMenuVariant1Props = {
  initialSpeed?: number
...
/**
 * Copy-and-own Tailwind component. Playback speed menu — a two-panel settings menu
 * with a checkmarked speed list and a navigable Quality submenu, both driven by real
 * selection state rather than a static always-open dropdown.
 */
export function PlaybackSpeedMenuVariant1({
  initialSpeed = 1,
  initialQuality = 'Auto (1080p)',
  onSpeedChange,
  onQualityChange,
  className,
}: PlaybackSpeedMenuVariant1Props) {
  ...
  return (
    <div className={`relative inline-block ${className ?? ''}`}>
      ...
      {panel === 'main' ? (
        <div role="menu" aria-label="Playback settings" className="absolute top-10 left-0 z-10 w-52 overflow-hidden rounded-lg border border-gray-200 bg-white py-1 shadow-lg">
          <p className="px-3 pt-1.5 pb-1 text-xs font-semibold text-gray-400">Playback speed</p>
        ...
        <div role="menu" aria-label="Quality options" className="absolute top-10 left-0 z-10 w-52 overflow-hidden rounded-lg border border-gray-200 bg-white py-1 shadow-lg">
          <button
            type="button"
            onClick={() => setPanel('main')}
            className="flex w-full items-center gap-1.5 px-3 py-1.5 text-sm font-semibold text-gray-900 hover:bg-gray-50"
          >
