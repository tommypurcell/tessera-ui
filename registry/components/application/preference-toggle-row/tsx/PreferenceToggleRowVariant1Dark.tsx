...
export type PreferenceToggleRowVariant1DarkProps = {
  preferences: PreferenceRow[]
  className?: string
}

/**
 * Copy-and-own Tailwind component (dark surface). Settings list where each row
 * pairs a label and description with a trailing switch. A locked row renders its
 * switch as non-interactive with reduced-opacity text.
 */
export function PreferenceToggleRow({ preferences, className }: PreferenceToggleRowVariant1DarkProps) {
  return (
        ...
        {preferences.map((pref) => (
          <li key={pref.id} className={`flex items-center gap-4 px-4 py-3.5 ${pref.locked ? 'opacity-50' : ''}`}>
            <div className="min-w-0 flex-1">
              {pref.locked ? (
                <span className="block text-sm font-medium text-gray-100">{pref.label}</span>
              ) : (
                <label htmlFor={pref.id} className="block text-sm font-medium text-gray-100">
                  {pref.label}
                </label>
              )}
            ...
            {pref.locked ? (
              ...
              <label
                className={`relative block h-7 w-12 shrink-0 rounded-full transition-colors [-webkit-tap-highlight-color:transparent] ${
                  pref.checked ? 'bg-green-500' : 'bg-gray-700'
                }`}
              >
                <input
                  type="checkbox"
