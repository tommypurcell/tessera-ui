function StarIcon({ filled }: { filled: boolean }) {
  return (
    <svg
      ...
      viewBox="0 0 20 20"
      fill="currentColor"
      className={`size-3.5 ${filled ? 'text-amber-400' : 'text-gray-200'}`}
    >
      <path
        fillRule="evenodd"
...
export type ProductCardVariant1Props = {
  imageSrc: string
  imageAlt: string
  title: string
  price: number
...
/**
 * Copy-and-own Tailwind component. E-commerce product card — image with a hover
 * quick-view overlay, title, star rating, price (with optional compare-at price), and an
 * add-to-cart action.
 */
export function ProductCardVariant1({
  imageSrc,
  imageAlt,
  title,
  price,
  ...
  return (
    <div className={`group overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm ${className ?? ''}`}>
      <div className="relative">
        ...
        {onQuickView ? (
          <div className="absolute inset-x-0 bottom-0 flex justify-center bg-gradient-to-t from-black/50 to-transparent p-3 opacity-0 transition-opacity group-hover:opacity-100">
            <button
              type="button"
              onClick={onQuickView}
      ...
      <div className="p-4">
        ...
        <div className="mt-2 flex items-center justify-between">
          ...
          <button
            ...
            onClick={onAddToCart}
            className="inline-flex size-8 items-center justify-center rounded-md bg-gray-900 text-white shadow-sm hover:bg-gray-800"
          >
