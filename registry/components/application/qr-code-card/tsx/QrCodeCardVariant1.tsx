...
export type QrCodeCardVariant1Props = {
  title: string
  value: string
  qrCode?: ReactNode
  onDownload?: () => void
...
/**
 * Copy-and-own Tailwind component. Card presenting a generated QR code with a
 * caption, the encoded value as visible text, and Download/Copy link actions.
 * Pass a real QR renderer via the `qrCode` prop; a static placeholder pattern
 * renders by default so the layout previews correctly without a QR dependency.
 */
export function QrCodeCard({ title, value, qrCode, onDownload, onCopyLink, className }: QrCodeCardVariant1Props) {
  return (
    <div className={`w-full rounded-xl border border-gray-200 bg-white p-5 text-center ${className ?? ''}`}>
      <div
        className="mx-auto flex h-40 w-40 items-center justify-center rounded-lg border border-gray-200 bg-white p-2"
        role="img"
      ...
      <div className="mt-4 flex gap-2">
        <button type="button" onClick={onDownload} className="flex-1 rounded-md border border-gray-300 px-3 py-1.5 text-xs font-medium text-gray-700 shadow-sm hover:bg-gray-50">
          Download
        </button>
        <button type="button" onClick={onCopyLink} className="flex-1 rounded-md border border-gray-300 px-3 py-1.5 text-xs font-medium text-gray-700 shadow-sm hover:bg-gray-50">
          Copy link
        </button>
      </div>
    </div>
