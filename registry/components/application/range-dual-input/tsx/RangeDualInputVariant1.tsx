...
export type RangeDualInputVariant1Props = {
  label: string
  min: number
  max: number
 ...
 */
export function RangeDualInput({ label, min, max, onMinChange, onMaxChange, floor = 0, prefix = '$' }: RangeDualInputVariant1Props) {
  const errorId = useId()
    ...
    <div>
      ...
      <div className="flex items-center gap-2">
        <label className="flex-1">
          ...
          <div className={`flex items-center rounded-md border bg-white shadow-sm ${fieldClasses}`}>
            {prefix ? (
            ...
            <input
              ...
              className="h-10 w-full border-0 bg-transparent px-2 text-sm text-gray-900 focus:outline-none focus:ring-0"
            />
          </div>
        </label>
