...
export type RatingSummaryHeaderVariant1DarkProps = {
  average: number
  totalReviews: number
}
 ...
 */
export function RatingSummaryHeaderDark({ average, totalReviews }: RatingSummaryHeaderVariant1DarkProps) {
      ...
      <div>
        <div className="flex items-center gap-0.5" role="img" aria-label={`Rated ${average.toFixed(1)} out of 5 stars`}>
          {Array.from({ length: 5 }, (_, index) => {
            ...
            if (index === fullStars && partialFraction > 0) {
              return (
                <span key={index} className="relative inline-block size-4">
                  <svg aria-hidden="true" className="absolute inset-0 size-4 text-gray-600" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                    <path fillRule="evenodd" d={starPath} clipRule="evenodd" />
                  </svg>
            ...
            return (
              <svg key={index} aria-hidden="true" className="size-4 text-gray-600" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                <path fillRule="evenodd" d={starPath} clipRule="evenodd" />
              </svg>
            )
          })}
        </div>
        <p className="mt-1 text-xs text-gray-400">Based on {totalReviews.toLocaleString()} reviews</p>
      </div>
    </div>
  )
}
