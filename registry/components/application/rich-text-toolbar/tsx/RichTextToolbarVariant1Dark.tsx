...
export type RichTextToolbarVariant1DarkProps = {
...
function ToolbarButton({
  ...
  return (
    <button
      ...
      className={
        pressed
          ? 'inline-flex size-8 items-center justify-center rounded-md bg-gray-800 text-sm text-white'
          : 'inline-flex size-8 items-center justify-center rounded-md text-sm text-gray-300 hover:bg-gray-800'
      }
    >
      {children}
    </button>
...
/**
 * Copy-and-own Tailwind component. WYSIWYG formatting bar adapted for dark surfaces, with a
 * heading select, toggled inline-format buttons, list buttons, and a link action.
 */
export function RichTextToolbarVariant1Dark({
  defaultActiveFormats = ['bold'],
  headingOptions = ['Paragraph', 'Heading 1', 'Heading 2', 'Heading 3'],
  onInsertLink,
  className,
  children,
}: RichTextToolbarVariant1DarkProps) {
  ...
  return (
    <div className={`rounded-lg border border-gray-800 bg-gray-950 shadow-sm ${className ?? ''}`}>
      <div role="toolbar" aria-label="Formatting" className="flex flex-wrap items-center gap-1 border-b border-gray-800 p-1.5">
        <label className="sr-only" htmlFor={headingId}>
          Text style
        </label>
        <select
          id={headingId}
          className="rounded-md border-0 bg-transparent py-1 pl-2 pr-6 text-sm font-medium text-gray-300 hover:bg-gray-800 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500"
        >
        ...
        <div role="separator" aria-orientation="vertical" className="mx-1 h-5 w-px bg-gray-800" />
        ...
        <button
          ...
          className="inline-flex size-8 items-center justify-center rounded-md text-gray-300 hover:bg-gray-800"
        >
          <svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="2" stroke="currentColor" className="size-4">
      ...
      <div className="p-4">
        {children ?? (
          <p className="text-sm leading-relaxed text-gray-300">
            <strong className="text-white">Ship on Friday</strong> only if the migration has
            been dry-run against staging. Otherwise, hold for Monday.
          </p>
        )}
      </div>
    </div>
