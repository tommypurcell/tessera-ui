export type SegmentedPageControlVariant1DarkProps = {
  count: number
  activeIndex: number
 ...
 */
export function SegmentedPageControlDark({ count, activeIndex, onChange, label = 'Slides' }: SegmentedPageControlVariant1DarkProps) {
  return (
      ...
      {Array.from({ length: count }, (_, index) => {
        ...
        return (
          <button
            ...
            className={`rounded-full transition-all ${isActive ? 'h-2 w-5 bg-white' : 'size-2 bg-gray-600'}`}
          />
        )
      })}
    </div>
