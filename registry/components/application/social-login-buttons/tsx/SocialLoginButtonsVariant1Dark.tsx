...
export type SocialLoginButtonsVariant1DarkProps = {
...
function ProviderIcon({ provider }: { provider: SocialProvider }) {
  ...
  if (provider === 'github') {
    return (
      <svg className="h-4 w-4 text-gray-100" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
        <path
          fillRule="evenodd"
...
/**
 * Copy-and-own Tailwind component (dark surface). Stacked set of full-width OAuth
 * provider buttons with brand-accurate icons and consistent sizing, plus an
 * optional "or continue with email" divider.
 */
export function SocialLoginButtons({
  ...
  className,
}: SocialLoginButtonsVariant1DarkProps) {
  return (
    <div className={`flex flex-col gap-6 ${className ?? ''}`}>
      <div className="flex flex-col gap-2.5">
        {providers.map((provider) => (
          <button
            ...
            className="flex w-full items-center justify-center gap-2.5 rounded-md border border-gray-700 bg-gray-900 px-4 py-2 text-sm font-medium text-gray-200 shadow-sm hover:bg-gray-800"
          >
      ...
      {showDivider ? (
        <div className="flex items-center gap-3">
          ...
          <span className="text-xs font-medium text-gray-500">{dividerLabel}</span>
          <span className="h-px flex-1 bg-gray-800" aria-hidden="true" />
        </div>
      ) : null}
    </div>
