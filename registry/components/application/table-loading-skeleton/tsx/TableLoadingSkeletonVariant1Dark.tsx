export type TableLoadingSkeletonVariant1DarkProps = {
  columns: string[]
  rowWidths: string[][]
  rowCount?: number
}
 ...
 */
export function TableLoadingSkeletonDark({ columns, rowWidths, rowCount = rowWidths.length }: TableLoadingSkeletonVariant1DarkProps) {
  return (
    <div className="overflow-hidden rounded-xl border border-gray-700">
      <table className="min-w-full divide-y divide-gray-700" role="status" aria-label="Loading table data">
            ...
            <tr key={rowIndex}>
              {columns.map((column, colIndex) => (
                <td key={column} className="px-4 py-3">
                  <div
                    className={`h-3.5 animate-pulse bg-gray-800 ${colIndex === 2 ? 'h-5 rounded-full' : 'rounded'}`}
