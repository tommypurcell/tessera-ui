...
export type TerminalCommandPaletteVariant1DarkProps = {
  title?: string
...
/**
 * Copy-and-own Tailwind component (true-black terminal surface). Inline command
 * runner: a scrollable history of prior command/output pairs above a live input
 * line with a blinking caret.
 */
export function TerminalCommandPalette({ title = 'Command palette', history, onRun, className }: TerminalCommandPaletteVariant1DarkProps) {
  ...
  return (
    <div className={`w-full overflow-hidden rounded-xl border border-gray-700 bg-black shadow-lg ${className ?? ''}`}>
      ...
      <div className="flex items-center gap-1.5 border-t border-gray-800 px-4 py-2.5 font-mono text-xs" onClick={() => inputRef.current?.focus()}>
        ...
        <input
          ...
          className="flex-1 bg-transparent text-gray-300 outline-none placeholder:text-gray-700"
