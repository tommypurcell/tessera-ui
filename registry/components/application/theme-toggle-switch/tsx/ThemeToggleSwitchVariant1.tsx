...
export type ThemeToggleSwitchVariant1Props = {
  value: ThemeOption
...
/**
 * Copy-and-own Tailwind component. Three-way Light/Dark/System theme selector
 * built as a real radio group: the selected option gets a white pill background
 * (transition-colors makes the pill feel animated) while the others sit flush
 * against the track. Distinct from the binary Toggles switch primitive.
 */
export function ThemeToggleSwitch({ value, onChange, name = 'theme', className }: ThemeToggleSwitchVariant1Props) {
  return (
    <fieldset className={`inline-flex rounded-lg bg-gray-100 p-1 ${className ?? ''}`} role="radiogroup" aria-label="Theme">
