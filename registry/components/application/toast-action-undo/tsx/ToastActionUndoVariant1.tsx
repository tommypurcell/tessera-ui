...
export type ToastActionUndoVariant1Props = Omit<HTMLAttributes<HTMLDivElement>, 'children'> & {
 ...
 */
export function ToastActionUndoVariant1({
  className,
  children,
  renderContent,
  before,
  ...
  errorContent,
  ...props
}: ToastActionUndoVariant1Props) {
  const defaultContent = (
    <>
      <div
            role="status"
            aria-live="polite"
            className="flex w-full items-center gap-3 rounded-lg border border-gray-800 bg-gray-900 py-3 pr-3 pl-4 text-white shadow-lg"
          >
            <div className="relative flex size-8 shrink-0 items-center justify-center">
              <svg className="size-8 -rotate-90" viewBox="0 0 32 32" aria-hidden="true">
                <circle cx="16" cy="16" r="13" fill="none" stroke="currentColor" strokeWidth="2.5" className="text-gray-700" />
                <circle
                  cx="16"
                  cy="16"
                  r="13"
