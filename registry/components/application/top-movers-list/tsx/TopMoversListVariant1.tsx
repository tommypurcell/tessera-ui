...
export type TopMoversListVariant1Props = {
  movers: MoverEntry[]
}
 ...
 */
export function TopMoversList({ movers }: TopMoversListVariant1Props) {
  return (
      ...
      {movers.map((mover) => {
        ...
        return (
          <li key={mover.symbol} className="flex items-center justify-between gap-4 px-4 py-3">
            <div>
              <p className="text-sm font-medium text-gray-900">{mover.symbol}</p>
              <p className="text-xs text-gray-500">{mover.name}</p>
            </div>
            <div className="text-right">
              <p className="font-mono text-sm font-semibold text-gray-900">{mover.value}</p>
