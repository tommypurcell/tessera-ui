...
export type VideoPlayerVariant1Props = Omit<HTMLAttributes<HTMLDivElement>, 'children'> & {
 ...
 */
export function VideoPlayerVariant1({
  className,
  children,
  renderContent,
  before,
  ...
  errorContent,
  ...props
}: VideoPlayerVariant1Props) {
  const defaultContent = (
    <>
<div className="relative w-full max-w-2xl overflow-hidden rounded-lg bg-gray-900 shadow-lg" style={{aspectRatio: '16 / 9'}}>
      ...
      <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent px-4 pb-3 pt-10">
        <div className="group relative mb-2 h-1 w-full cursor-pointer rounded-full bg-white/25">
          <div className="absolute inset-y-0 left-0 w-1/3 rounded-full bg-white/40"></div>
