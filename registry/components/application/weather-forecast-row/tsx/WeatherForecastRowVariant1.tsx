...
const iconPaths: Record<DailyForecast['icon'], { path: string; className: string }> = {
  sun: { path: 'M12 3v1.5m0 15V21m9-9h-1.5M4.5 12H3m15.36 6.36-1.06-1.06M6.7 6.7 5.64 5.64m12.72 0-1.06 1.06M6.7 17.3l-1.06 1.06M16.5 12a4.5 4.5 0 1 1-9 0 4.5 4.5 0 0 1 9 0Z', className: 'text-amber-400' },
  cloud: { path: 'M2.25 15a4.5 4.5 0 0 0 4.5 4.5H18a3.75 3.75 0 0 0 1.332-7.257 3 3 0 0 0-3.758-3.848 5.25 5.25 0 0 0-10.233 2.33A4.502 4.502 0 0 0 2.25 15Z', className: 'text-gray-400' },
...
/**
 * Copy-and-own Tailwind component. 7-day forecast strip with per-day
 * icon, hi/lo temperatures, and precipitation percentage — distinct from
 * weather-widget, which is a current-conditions tile with an hourly
 * strip and no multi-day forecast or precipitation data.
 */
export function WeatherForecastRow({ forecast }: WeatherForecastRowVariant1Props) {
  return (
    <div className="flex w-full max-w-2xl divide-x divide-gray-100 rounded-lg border border-gray-200 bg-white">
      {forecast.map((day) => {
        ...
        return (
          <div key={day.day} className="flex flex-1 flex-col items-center gap-1.5 px-2 py-4">
            ...
            <span className="text-[11px] text-blue-500">{day.precipPercent}%</span>
