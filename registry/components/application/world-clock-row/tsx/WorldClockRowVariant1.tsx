...
export type WorldClockRowVariant1Props = {
 ...
 */
export function WorldClockRow({ entries }: WorldClockRowVariant1Props) {
  return (
      ...
      {entries.map((entry) => (
        <li key={entry.city} className="flex items-center justify-between gap-4 px-4 py-3">
          <div className="flex items-center gap-2.5">
            <svg
              aria-hidden="true"
              className={`size-4 ${entry.isDaytime ? 'text-amber-500' : 'text-indigo-500'}`}
            ...
            <div>
              <p className="text-sm font-medium text-gray-900">{entry.city}</p>
              <p className="text-xs text-gray-500">{entry.zoneLabel}</p>
            </div>
          </div>
          <p className="font-mono text-sm font-semibold text-gray-900">{entry.time}</p>
        </li>
      ))}
    </ul>
  )
